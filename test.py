...
class Tester(object):
    def __init__(self, args):
        ...
        if lpips is not None:
            ...
            print("Warning: lpips is not installed, LPIPS metric will be skipped.")
        ...
        # Keep original resolution if user explicitly disables resizing.
        if args.no_resize:
            args.resize = False
    ...
    def testing(self):
        ...
        with torch.no_grad():
            ...
            for _, (x, label, fn) in loop:
                ...
                if self.lpips_metric is not None:
                    ...
                    label_lpips = label_t * 2.0 - 1.0
                    lpips_batch = self.lpips_metric(pred_lpips, label_lpips)
                    lpips_sum += lpips_batch.view(lpips_batch.size(0), -1).mean(dim=1).sum().item()
                    lpips_count += lpips_batch.size(0)
        ...
        lpips_ = lpips_sum / lpips_count if lpips_count > 0 else float("nan")

        print(
            # "[Testing] SSIM: %.4f, PSNR: %.4f, UIQM: %.4f, UCIQE: %.4f"
            "[Testing] SSIM: %.4f, PSNR: %.4f, LPIPS: %.4f"
            % (ssim_, psnr_, lpips_)
            # % (ssim_, psnr_, uiqm_, uciqe_)
        )
        with open("/".join(self.args.ckpt.split("/")[:-1]) + "/result.txt", "w") as f:
            f.write("[Testing] SSIM: %.4f, PSNR: %.4f, LPIPS: %.4f" % (ssim_, psnr_, lpips_))
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Do not resize test images; keep original resolution.",
    )
